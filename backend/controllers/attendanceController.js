...
exports.adminListAttendance = async (req, res) => {
    try {
        ...
        if (type) query.type = type; // Now filter by type instead of status
        if (from || to) {
            query.timestamp = {}; // Use timestamp instead of createdAt
            if (from) query.timestamp.$gte = new Date(from);
            if (to) query.timestamp.$lte = new Date(to);
        }
        const records = await Attendance.find(query).populate('userId', 'name email').sort({ timestamp: -1 });
